...
function Header({ onSearch }) {
    ...
    const [selectedProvince, setSelectedProvince] = useState({ id: "1", name: "Hà Nội" }); // Tỉnh được chọn với id và name
    ...
    const categories = ["Nhà hàng", "Quán ăn", "Cafe", "Tiệm bánh"];
    ...
    useEffect(() => {
      fetch("https://open.oapi.vn/location/provinces?page=0&size=64")
        ...
        .then((data) => {
          if (data.data) {
            setProvinces(data.data); // Lưu danh sách tỉnh vào state
          }
        })
        .catch((error) => {
    ...
    useEffect(() => {
      fetch(
        `https://open.oapi.vn/location/districts/${selectedProvince.id}?page=0&size=30`
      )
        .then((response) => response.json())
        .then((data) => {
          if (data.data) {
            setDistricts(data.data); // Lưu danh sách quận/huyện vào state
          }
        })
        .catch((error) => {
  ...
  // Fetch API lấy danh sách quận/huyện khi tỉnh thành được chọn
  useEffect(() => {
    fetch(`https://open.oapi.vn/location/districts/${selectedProvince.id}?page=0&size=30`)
      .then((response) => response.json())
      .then((data) => {
        if (data.data) {
          setDistricts(data.data); // Lưu danh sách quận/huyện vào state
    ...
    <>
      ...
      >
        ...
        <a
          href="/"
          className={`me-3 text-light text-decoration-none nav-link ${
            window.location.pathname === "/dat-giao-hang"
              ? "active-custom"
              : ""
          }`}
        >
                ...
                {provinces.map((province) => (
                  <li key={province.id}>
                    <button
                      ...
                      onClick={() => setSelectedProvince({ id: province.id, name: province.name })}
                    >
                      {province.name}
                    </button>
                  </li>
  ...
  >
    ...
    >
      <i className="bi bi-filter" style={{ marginRight: "4px" }}></i>Bộ lọc
    </button>
        ...
        {activeTab === "Khu vực" && (
          <div className="row mb-3">
            {districts.map((district) => (
              <div key={district.id} className="col-6 col-sm-6 col-md-6 mb-2">
  ...
  <div className="dropdown">
    ...
    >
      <strong>
        {userEmail.length > 12 ? `${userEmail.substring(0, 12)}...` : userEmail}
      </strong>
    </button>
